...
func TestGetInterval(t *testing.T) {
	...
	for i := 1; i <= 16; i++ {
		...
		t.Run(testName, func(t *testing.T) {
			...
			if err := interval.Start(context.Background(), config, noop, noop, noop); err != nil {
				t.Fatal(err)
			}

			if interval.Category != expCategory {
				t.Errorf("Expected category %q, got %q.\n",
					expCategory, interval.Category)
			}

			if interval.PlannedDuration != expDuration {
				t.Errorf("Expected PlannedDuration %q, got %q.\n",
					expDuration, interval.PlannedDuration)
			}

			if interval.State != pomodoro.StateNotStarted {
				t.Errorf("Expected State = %q, got %q.\n",
					pomodoro.StateNotStarted, interval.State)
			}

			ui, err := repo.ByID(interval.ID)
			if err != nil {
				t.Errorf("Expected no error. Got %q.\n", err)
			}
			...
			if ui.State != pomodoro.StateDone {
				t.Errorf("Expected State = %q, got %q.\n",
					pomodoro.StateDone, interval.State)
			}
		})
	}
}
